...
export async function embedContent(content: string): Promise<Vector> {
    const result = await embeddingPipeline(content, { pooling: 'mean', normalize: true });
...
export async function getRecommendedStories(userId: string, k: number = 10): Promise<Story[]> {
    ...
    let weightedSum = Array(1024).fill(0);  // bge-large-en-v1.5 produces 1024-dimensional embeddings
    let weightSum = 0;
    
    for (const historyEntry of userHistory) {
        const timeDiff = (currentTime.getTime() - historyEntry.readTime.getTime()) / (1000 * 60 * 60 * 24);
        ...
        if (historyEntry.story.embedding) {
            ...
            weightedSum = weightedSum.map((val, i) => val + weight * (embedding[i] ?? 0));
            weightSum += weight;
        } else {
    ...
    for (const story of allStories) {
        if (!userHistory.some(h => h.storyId === story.id)) {
            const simScore = computeCosineSimilarity(userProfile, JSON.parse(story.embedding));
    ...
    const diverseRecommendations = await addDiversity(recommendations, allStories, k);
...
async function addDiversity(recommendations: [string, number][], stories: Story[], k: number): Promise<[string, number][]> {
    invariant(recommendations.length > 0, 'Recommendations array must contain at least one element');
    invariant(stories.length > 0, 'Stories array must contain at least one element');
    invariant(k > 0, 'k must be greater than 0');
	invariant(recommendations[0], 'First recommendation must exist');
	const diverseRecs: [string, number][] = recommendations[0] ? [recommendations[0]] : [];
    ...
    for (let i = 1; i < k; i++) {
        ...
        for (const rec of recommendations) {
            if (!diverseRecs.some(r => r[0] === rec[0])) {
                const minDistance = Math.min(...diverseRecs.map(r => 
                    computeCosineSimilarity(JSON.parse(storyMap.get(rec[0])!.embedding), JSON.parse(storyMap.get(r[0])!.embedding))
                ));
...
export async function action({ request }: ActionFunctionArgs) {
    ...
    const storyId = await addNewStory(content);
